...
type Tile = { name: string; kind: string; text: string; href: string; label: string }

const TILES: Tile[] = [
  { name: 'Ledger', kind: 'track',
    text: "Signs every transaction. The pendant speaks the Nano X's Bluetooth transport itself, ported to the ESP32, and sends EIP-712 typed data so the device clear-signs the same words the wrist saw. Measuring it produced an upstream contribution.",
    href: `${REPO}/blob/main/docs/LEDGER.md`, label: 'docs/LEDGER.md · how it is used' },
  { name: 'The Graph', kind: 'track',
    text: 'Remembers every decision. Our own subgraph indexes AmuletLog; the dashboard reads it, and so does each agent before it proposes, learning from its own refusals. The market it reasons over comes from the Aave, Compound and Spark subgraphs.',
    href: `${REPO}/blob/main/docs/THE_GRAPH.md`, label: 'docs/THE_GRAPH.md · how it is used' },
  { name: 'ENS', kind: 'track',
    text: 'Names the agents and holds the rules. repay, yield and swap each live under amuletguard.eth with a cap and an allow-list only a Ledger signature can edit. The pendant reads those records itself, straight off the resolver.',
    href: `${REPO}/blob/main/docs/ENS.md`, label: 'docs/ENS.md · how it is used' },
