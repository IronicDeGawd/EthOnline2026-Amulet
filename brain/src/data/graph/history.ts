...
const QUERY = `query($id: String!, $n: Int!) {
  agent(id: $id) { id label requests approved rejected refusedByPolicy expired }
  actions(first: $n, orderBy: timestamp, orderDirection: desc, where: { agentLabel: $id }) {
...
export async function fetchHistory(label: string, n = 5, url = AMULET_SUBGRAPH, timeoutMs = 8000): Promise<AgentHistory | undefined> {
  try {
    const res = await fetch(url, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ query: QUERY, variables: { id: label, n } }),
      signal: AbortSignal.timeout(timeoutMs),
    });
    if (!res.ok) return undefined;
    const j = (await res.json()) as { data?: { agent?: Record<string, unknown> | null; actions?: Record<string, unknown>[] } };
    const a = j.data?.agent;
    const rows = j.data?.actions ?? [];
    if (!a && !rows.length) return undefined;
    return {
      label,
      requests: Number(a?.requests ?? rows.length),
      approved: Number(a?.approved ?? 0),
      rejected: Number(a?.rejected ?? 0),
      refusedByPolicy: Number(a?.refusedByPolicy ?? 0),
      expired: Number(a?.expired ?? 0),
      recent: rows.map((r) => ({
        outcome: String(r.outcome), value: String(r.value), target: String(r.target), timestamp: Number(r.timestamp),
      })),
    };
  } catch {
    return undefined;
...
export function historyLines(h: AgentHistory | undefined, now = Date.now()): string {
  ...
  const lines = h.recent
    ...
    .map((r) => {
      ...
      return `  ${ago(Math.max(0, now / 1000 - r.timestamp))}: you asked for ${eth} ETH — they ${SAID[r.outcome] ?? r.outcome}\n`;
    })
    .join("");
