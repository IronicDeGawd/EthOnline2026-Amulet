...
export class Brain {
  ...
  async tick(): Promise<boolean> {
    ...
    if (d.decider) {
      ...
      const spendable = d.dep.amuletAccount ? await d.sepolia.getBalance({ address: d.dep.amuletAccount }) : undefined;
      const j = await d.decider.decide(pos, market, d.policy, d.mandate ?? "", fresh.evidence, hist, this.lastRefusal, undefined, spendable);
